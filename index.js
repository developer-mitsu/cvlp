...
document.addEventListener('DOMContentLoaded', function(){
    ...
    for(var i = 0; i < 2; i++){
        document.getElementById("js-validate"+(i+1)).addEventListener('click', function(e) {
            e.preventDefault();
            var formElements = [
                {
                    name: "氏名",
                ...
                {
                    name: "電話番号",
                    element: document.getElementsByName('phone')[0],
                    submit_flg: false
                },
                {
                    name: "メールアドレス",
                ...
                {
                    name: "カウンセリング希望校",
                    element: document.getElementsByName('venue')[0],
                    submit_flg: false
                },
                {
                    name: "希望日程",
            ...
            var validate= function(formElement) {
                if (!formElement.element.value.length > 0) {
                    return formElement.name;
                } else {
                    formElement.submit_flg = true;
                }    
            }
            ...
            for (var i = 0; i < formElements.length; i++ ){
                if (!formElements[i].submit_flg){
                    window.alert(message);
                    message += validate(formElements[i])+"\n";
                    continue;
                }
                if (formElements[formElements.length - 1].submit_flg) {
                    form.submit();
                } else if (!formElements[formElements.length - 1].submit_flg) {
                    window.alert(message);
                }
            }
